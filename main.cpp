...
const float COEF_INTEGRAL_YAW = 0.0;const float COEF_DIFFERENTIAL_YAW = 0.f;
const float COEF_PROPORTIONAL_X = 0.5;
const float COEF_INTEGRAL_X = 0.0;const float COEF_DIFFERENTIAL_X = 0.f;
...
#pragma omp section
    ...
    //=====================================================================================
    // Two threads:
    // first thread is simulation the Stroller control unit
    // second thread is the working algorithm
    for (;;)
    {
...
static void prvRiscBehavior(TEnumRiscBehavior eErrorCode, string sError)
{
  ...
  for (;;)
  {
    cout << " = = = Orientation calculation terminated = = = " << endl;
    this_thread::sleep_for(5000ms);
  }
}
